...
class CSPLayer2(nn.Module):
    def __init__(
        ...
        hidden_channels = int(out_channels * expansion)
        self.conv1 = ConvNormLayer_fuse(
            in_channels, hidden_channels * 2, 1, 1, bias=bias, act=act
        )
        self.bottlenecks = nn.Sequential(
            *[
                bottletype(hidden_channels, hidden_channels, act=act)
...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        Fuse_Block = RepNCSPELAN4(
            c1=c1, c2=c2, c3=c3, c4=c4, n=num_blocks, act=act, csp_type=csp_type
        )
    ...
    def forward(self, proj_feats):
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            fused_feat = (
                ...
                if self.fuse_op == "sum"
                else torch.concat([upsample_feat, feat_low], dim=1)
            )
